<script lang="ts">
    ...
    const lobbyID = $derived(room?.roomId ?? '');
    const isHost = $derived(room?.sessionId === $gameStore.hostID);
</script>

<main class="flex min-h-screen items-center justify-center">
    <Card.Root>
        <Card.Header>
            <Card.Title>Waiting for game to start ({$gameStore.players.size}/{$gameStore.maxPlayers})</Card.Title>
